...
{
	return (vid == 0x16D0 && (pid == 0x127E || pid == 0x1460)) 
...
static char kr_fn_table[] =
{
	...
	KEY_LEFTBRACE,  KEY_F13,    
	KEY_RIGHTBRACE, KEY_F14,    
	KEY_DOT,        KEY_KPDOT,
...
static int keyrah_trans(int key, int press)
{
	...
	if (key == KEY_NUMLOCK)    return KEY_F13; 
	if (key == KEY_SCROLLLOCK) return KEY_F14; 
	if (key == KEY_INSERT)     return KEY_F16; 
...
static void input_cb(struct input_event *ev, struct input_absinfo *absinfo, int dev, bool menu_event)
{
	...
	if (mapping && mapping_type == 3 && ev->type == EV_KEY)
	{
		...
		if (ev->value == 1)
		{
			...
			if (mapping_button == sizeof(mapping_store->input_codes)/sizeof(mapping_store->input_codes[0])) 
				mapping_button--;
			if (mapping_button > 1)
								...
								if (!found || (mapping_button == SYS_BTN_OSD_KTGL && mapping_type))
								{
									...
									if (mapping_button == BUTTON_DPAD_COUNT + 1 || mapping_button == BUTTON_DPAD_COUNT + 3)
									{
										if (input[dev].map[mapping_button] >= KEY_EMU &&
											...
											(input[dev].map[mapping_button - 1] - input[dev].map[mapping_button] == 1) && 
											absinfo)
										{
...
void mergedevs()
{
	...
	make_unique(0x289B, 0x0057, -1); 
	make_unique(0x0E8F, 0x3013, 1);  
	make_unique(0x16C0, 0x05E1, 1);  
	make_unique(0x045E, 0x02A1, 1);  
	make_unique(0x8282, 0x3201, 1);  
	make_unique(0x1209, 0xFACA, 1);  
	make_unique(0x16D0, 0x127E, 1);  
	make_unique(0x16D0, 0x1460, 1);  
	make_unique(0x1209, 0x595A, 1);  
...
} jamma2joy[] =
{
	...
	{KEY_LEFT,      1, 0x124}, 
	{KEY_RIGHT,     1, 0x125}, 
	{KEY_LEFTCTRL,  1, 0x126}, 
	{KEY_LEFTALT,   1, 0x127}, 
	{KEY_SPACE,     1, 0x128}, 
	{KEY_LEFTSHIFT, 1, 0x129}, 
	{KEY_Z,         1, 0x12A}, 
	{KEY_X,         1, 0x12B}, 
	{KEY_C,         1, 0x12C}, 
	{KEY_V,         1, 0x12D}, 
...
static struct
{
    ...
    uint16_t btn;
} jamma22joy[] =
        {
                ...
                {KEY_LEFT,      3, 0x124}, 
                {KEY_RIGHT,     3, 0x125}, 
                {KEY_LEFTCTRL,  3, 0x126}, 
                {KEY_LEFTALT,   3, 0x127}, 
                {KEY_SPACE,     3, 0x128}, 
                {KEY_LEFTSHIFT, 3, 0x129}, 
                {KEY_Z,         3, 0x12A}, 
                {KEY_X,         3, 0x12B}, 
                {KEY_C,         3, 0x12C}, 
                {KEY_V,         3, 0x12D}, 
...
static int rumble_input_device(int devnum, uint16_t strong_mag, uint16_t weak_mag, uint16_t duration = 500, uint16_t delay = 0)
{
	...
	if (!strong_mag && !weak_mag) 
	{
		if (input[devnum].rumble_effect.id == -1) return 1; 

		ioret = ioctl(fd, EVIOCRMFF, input[devnum].rumble_effect.id);
		input[devnum].rumble_effect.id = -1; 
		return ioret != -1;
	}
	else {
				...
				if ((pool[i].fd >= 0) && (pool[i].revents & POLLIN))
				{
								...
								if (input[i].quirk == QUIRK_MADCATZ360 && ev.type == EV_KEY)
								{
									...
									else if (ev.code == BTN_MODE && !user_io_osd_is_visible())
									{
										if (input[i].misc_flags & 1)
										{
											if (ev.value)
											{
												if ((input[i].misc_flags & 0x6) == 0) input[i].misc_flags = 0x3; 
												else if ((input[i].misc_flags & 0x6) == 2) input[i].misc_flags = 0x5; 
												else input[i].misc_flags = 0x1; 
					...
					else
					{
						...
						if (read(pool[i].fd, data, sizeof(data)))
						{
							int edev = i;
							int dev = i;
							if (input[i].bind >= 0) edev = input[i].bind; 
							if (input[edev].bind >= 0) dev = input[edev].bind; 
			...
			if ((pool[NUMDEV + 1].fd >= 0) && (pool[NUMDEV + 1].revents & POLLIN))
			{
				...
				if (len)
				{
						...
						(cmd[10] == '\0' || cmd[10] == ' ' || cmd[10] == '\t'))
					{
						...
						physical_disc_set_device(dev);

						
						
						physical_disc_mount_current_core();
					}
					else if (!strncmp(cmd, "swap_disc", 9) &&
						(cmd[9] == '\0' || cmd[9] == ' ' || cmd[9] == '\t'))
					{
						...
						physical_disc_swap_current_core();
					}
				}
			}
...
bool update_advanced_state(int devnum, uint16_t evcode, int evstate)
{
	...
	for (uint i = 0; i < ADVANCED_MAP_MAX; i++)
	{
		...
		{
			if (abs->pressed && chord_has_osd && pnum > 0) osd_autofire_consumed[pnum-1] = true; 
